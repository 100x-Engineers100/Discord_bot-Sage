"""
Insert 1050 demo queries for Apr 2026 analytics demo.
Window: Mar 6 – Apr 5, 2026

Distribution:
  - ~900 queries across Mar 6 – Mar 28  (first 23 days)
  - ~150 queries across Mar 29 – Apr 5  (last 7 days)
...
All rows tagged is_demo6=true. Does NOT touch existing production data.

Cleanup SQL (run after demo):
  DELETE FROM analytics_events WHERE metadata->>'is_demo6' = 'true';
"""
...
EARLY_START  = datetime(2026, 3,  6,  0,  0,  0, tzinfo=timezone.utc)  # Mar 6
EARLY_END    = datetime(2026, 3, 28, 23, 59, 59, tzinfo=timezone.utc)  # Mar 28  (23 days)
RECENT_START = datetime(2026, 3, 29,  0,  0,  0, tzinfo=timezone.utc)  # Mar 29
RECENT_END   = datetime(2026, 4,  5, 23, 59, 59, tzinfo=timezone.utc)  # Apr 5   (8 days)

EARLY_DAYS  = 23
RECENT_DAYS = 8

BASE_THREAD_ID = 2000000000000000000
BASE_USER_ID   = 8500000000000000000
NUM_USERS      = 55
...
N_NO_ENG      = TOTAL_QUERIES - N_TAG_CREW - N_GOT_IT - N_CONTINUE  # 349

N_RECENT = 150  # queries in last 7 days
N_EARLY  = TOTAL_QUERIES - N_RECENT  # 900
...
print(f'[*] Planned: {TOTAL_QUERIES} queries | '
      ...
      f'last-7-day queries {N_RECENT}')

# ---------------------------------------------------------------------------
# Query pool (305 unique; cycled to reach 1050)
# ---------------------------------------------------------------------------
...
def random_ts_in(rng: random.Random, start: datetime, span_days: int, end: datetime) -> datetime:
...
def build_records() -> list[dict]:
    ...
    # Build timestamp list: 900 early + 150 recent, shuffled together
    timestamps = []
    for _ in range(N_EARLY):
        timestamps.append(random_ts_in(rng, EARLY_START, EARLY_DAYS, EARLY_END))
    for _ in range(N_RECENT):
        timestamps.append(random_ts_in(rng, RECENT_START, RECENT_DAYS, RECENT_END))
...
def main():
    print('=' * 60)
    print('Building 1050 demo records (Mar 6 – Apr 5, 2026)')
    print('Production data is NOT touched.')
    print('=' * 60)
    ...
    # Count how many queries fall in last 7 days
    recent_count = sum(
        1 for r in records
        if r['event_type'] == 'query'
        and r['created_at'] >= RECENT_START.isoformat()
    )
    ...
    print(f'     last-7-day q   : {recent_count}  (target ~150)')
    print()
    ...
    print('[OK] Done.')
    print()
    print('Cleanup SQL (run after demo):')
    print("  DELETE FROM analytics_events WHERE metadata->>'is_demo6' = 'true';")
